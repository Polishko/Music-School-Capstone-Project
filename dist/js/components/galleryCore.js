...
import { displayMessage, enrollInCourse, filterCourses, disableCourseEnroll, getEnrolledCourses, getSessionStoredFilters, searchCourses, sortCourses, toggleDetailsParagraph } from "../utils.js";
export function loadCourses() {
    fetch("data/courses.json")
        .then((response) => {
        if (!response.ok) {
            displayMessage("gallery", "p", "message error", "Sorry, we couldn`t load the courses. Please try again later.");
        ...
        if (data.length) {
            ...
            const { sort } = filters, filterCriteria = __rest(filters, ["sort"]);
            if (sort === "title-asc" ||
                sort === "instructor-asc" ||
                sort === "level-asc") {
            ...
            const searchTerm = inputElement === null || inputElement === void 0 ? void 0 : inputElement.value.trim().toLowerCase();
            if (searchTerm) {
            ...
            if (!processedData.length) {
                ...
                renderPaginationData(processedData, totalPages, currentPage, itemsPerPage);
            }
        }
        else {
            displayMessage("gallery", "p", "message info", "No courses available yet.");
        }
    })
        .catch((error) => {
        console.error("Error loading course:", error);
        displayMessage("gallery", "p", "message error", "Oops! Something went wrong while loading the courses.");
    });
}
